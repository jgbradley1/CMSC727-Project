...
for forgetSize = 0:10:500
    ...
    horizontalX(counter) = forgetSize;
    counter = counter+1;
    
    disp (['Forget Size: ', num2str(forgetSize), '  Mackey Error: ', num2str(mackeyErrorRate(counter-1)), '  Power Error: ', num2str(powerErrorRate(counter-1)), '  Stock Error: ', num2str(stockErrorRate(counter-1))]);
end
...
title('Forget Size Optimization');
xlabel('Forget Size');
ylabel('MSE');
...
axis([-5 505 0 maxError+0.05*maxError])
